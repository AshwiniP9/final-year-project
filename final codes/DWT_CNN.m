...
% 
% % % Classification using CNN networks
% for i = 1:kfold
% X1=[Z1(:,1:80),S1(:,1:80)];
% X2=reshape(X1',160,L);
% Xtrain = mat2cell(X2,rowdist1);
...
% end
% Acc1  = mean(Afold);
% fprintf("\n Accuracy of classification between ictal and normal(B) = %f \n",100*Acc1);
% figure;plotconfusion(Tvalidate,YPred1);
% title(' Confusion Matrix B - E ')
% % % % ---------------------------------------------------------------------------------------------------------------------------------------%
% % % Classification between interictal(C) and ictal(E) with db4
%  L=3028;
%  Z1=zeros(L,100);
%  S1=zeros(L,100);
% for i=1:100
...
% end
% Acc1  = mean(Afold);
% fprintf("\n Accuracy of classification between ictal and interictal(C) = %f \n",100*Acc1);
% figure;plotconfusion(Tvalidate,YPred1);
% title(' Confusion Matrix C - E ')
% % [TPR3,TNR3,Acc3,BA3,PPV3,NPV3,F13,MCCS3] = myperf(Tvalidate,YPred3);
% % % ---------------------------------------------------------------------------------------------------------------------------------------%
% % % Classification between interictal(D) and ictal(E)  with db4
%  L=3028;
%  Z1=zeros(L,100);
%  S1=zeros(L,100);
% for i=1:100
...
% end
% Acc1  = mean(Afold);
% fprintf("\n Accuracy of classification between ictal(E) and normal(ABCD) = %f \n",100*Acc1);
% figure;plotconfusion(Tvalidate,YPred1);
...
Acc1  = mean(Afold);
fprintf("\n Accuracy of classification between ictal(E) and interictal(CD) = %f \n",100*Acc1);
figure;plotconfusion(Tvalidate,YPred1);
...
% 
% c1 = confusionmat(Tvalidate,YPred1);
% c=c1';
